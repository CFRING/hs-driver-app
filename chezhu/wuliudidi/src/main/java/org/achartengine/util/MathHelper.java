...
public class MathHelper {
  ...
  private static double[] computeLabels(final double start, final double end,
    ...
    /**原代码里多用了一个roundUp函数，由此y轴上刻度的跨度，就不再是（上限-下限）/刻度数目了，然后它自己得出了这个跨度，就会在坐标轴上相应的地方打上label，
     * 我手工插入的label与它自己打的label如果不在同一个地方，它自己打的label就会显示出来。*/
    //double xStep = roundUp(Math.abs(s - e) / approxNumLabels);
    double xStep = Math.abs(s - e) / approxNumLabels;
