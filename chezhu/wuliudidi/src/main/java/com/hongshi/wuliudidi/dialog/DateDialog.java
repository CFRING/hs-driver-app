...
public class DateDialog extends Dialog implements OnWheelChangedListener {
    ...
    private void init() {
        ...
        if(dateType.equals(Entire)){
            ...
            mHourView.setDrawShadows(false);
            mMinuteView.setDrawShadows(false);
        ...
        for(int i = 0;i <= 60; i += 5){
            if (Integer.valueOf(minuteStr) > i && Integer.parseInt(minuteStr) <i+5){
                minuteStr = String.valueOf(i + 5);
                if (minuteStr.equals("60")){
                    minuteStr = "00";
                    hourStr = String.valueOf(Integer.parseInt(hourStr) + 1);
                }
                break;
            }
        }
