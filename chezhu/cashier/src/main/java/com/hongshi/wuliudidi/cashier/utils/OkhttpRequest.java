...
public class OkhttpRequest {
    ...
    public static void postString(final Context context, String url, Map<String, String> params, final RequestCallback mRequestCallback){
        try {
            com.hongshi.wuliudidi.cashier.okhttp.OkHttpUtils.getInstance().setCertificates(context.getAssets().open("HSWLROOTCAforInternalTest.crt"),context.getAssets().open("HSWLROOTCA.crt"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        com.hongshi.wuliudidi.cashier.okhttp.OkHttpUtils.post().url(url).params(params).build().execute(new Callback<String>() {
            @Override
